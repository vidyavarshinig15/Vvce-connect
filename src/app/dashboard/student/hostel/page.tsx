...
export default function StudentHostelDashboard() {
  ...
  }, [userProfile?.email, supabase]);

  const handleRaiseComplaint = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!allocation || !userProfile?.email) return;
  ...
  return (
    <div className="max-w-5xl mx-auto pb-12">
      ...
      <div className="bg-[#2E8B57] rounded-3xl p-6 md:p-8 text-white shadow-lg mb-8 flex flex-col md:flex-row items-start md:items-center justify-between gap-6 border border-emerald-700">
        <div>
          ...
          <div className="flex items-center gap-4 mt-3">
            <span className="bg-white/20 px-4 py-1.5 rounded-lg font-bold text-sm flex items-center gap-2">
      ...
      <div className="flex gap-4 mb-8 border-b border-slate-200 pb-px">
        ...
        <button onClick={() => setActiveTab('announcements')} className={`pb-4 font-bold text-sm px-2 flex items-center gap-2 ${activeTab === 'announcements' ? 'text-[#2E8B57] border-b-2 border-[#2E8B57]' : 'text-slate-400 hover:text-slate-600'}`}>
          <Bell size={18}/> Warden Announcements {announcements.length > 0 && <span className="bg-red-500 text-white text-[10px] px-2 py-0.5 rounded-full">{announcements.length}</span>}
        </button>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-1">
            <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm">
              <h3 className="text-lg font-black text-slate-800 mb-6 flex items-center gap-2">
                <PlusCircle className="text-[#2E8B57]" size={20} /> Raise an Issue
              </h3>
              <form onSubmit={handleRaiseComplaint} className="space-y-4">
                <div className="space-y-1">
                  ...
                  <select required value={complaintForm.issue_type} onChange={e=>setComplaintForm({...complaintForm, issue_type: e.target.value})} className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:border-[#2E8B57] font-semibold text-slate-700">
                ...
                <div className="space-y-1">
                  ...
                  <textarea required rows={4} value={complaintForm.description} onChange={e=>setComplaintForm({...complaintForm, description: e.target.value})} className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:border-[#2E8B57] resize-none" placeholder="E.g., The food quality today was poor..." />
                </div>
                <button type="submit" className="w-full bg-[#2E8B57] text-white font-black py-4 rounded-xl hover:bg-[#257046] mt-2 shadow-lg shadow-emerald-600/20">
                  Submit to Warden
                </button>
              </form>
            </div>
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            {myComplaints.length === 0 ? (
              <div className="bg-slate-50 border border-dashed border-slate-300 rounded-3xl p-12 text-center text-slate-500">
              ...
              <div key={c.id} className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                <div className="flex justify-between items-start mb-3">
                  <div className="flex items-center gap-2">
                    <span className="bg-slate-100 text-slate-700 font-black px-3 py-1 rounded text-[10px] uppercase tracking-widest border border-slate-200">{c.issue_type}</span>
                ...
                {c.warden_response ? (
                  <div className="bg-emerald-50/50 p-4 rounded-xl border border-emerald-100">
                    <p className="text-[10px] font-black text-[#2E8B57] uppercase tracking-widest mb-1 flex items-center gap-1"><MessageSquare size={12}/> Message from Warden</p>
      ...
      {activeTab === 'announcements' && (
        <div className="max-w-3xl space-y-4">
          ...
          {announcements.length === 0 ? (
            ...
            <div key={a.id} className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm relative overflow-hidden">
              <div className="absolute top-0 left-0 w-1 h-full bg-[#2E8B57]"></div>
              <div className="flex justify-between items-start mb-3">
                <h4 className="text-xl font-black text-slate-800">{a.title}</h4>
                <span className="text-[10px] text-slate-400 font-black uppercase tracking-widest bg-slate-50 px-3 py-1 rounded-lg border">
