...
export default function WardenProfilePage() {
  ...
  const [profile, setProfile] = useState({
    id: '', full_name: '', email: '', phone: '', branch: '', role: 'warden'
  });
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      const { error } = await supabase.from('profiles').update({
        ...
        phone: profile.phone,
        branch: profile.branch // Branch for warden might be "Hostel Dept" or similar
      }).eq('id', profile.id);
  ...
  return (
    <div className="max-w-4xl mx-auto pb-12">
      <div className="flex justify-between items-end mb-8">
        <div>
          <h1 className="text-3xl font-black text-slate-900 uppercase tracking-tight">Warden Profile</h1>
          <p className="text-sm text-slate-500 mt-1">Manage your administrative contact details.</p>
        </div>
        {!isEditing && (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-1">
          <div className="bg-white rounded-3xl p-8 shadow-sm border border-slate-200 flex flex-col items-center text-center">
            ...
            <p className="text-[10px] font-black uppercase tracking-widest text-[#2E8B57] mt-1">Authorized Warden</p>
            <div className="mt-6 w-full pt-6 border-t border-slate-100 space-y-3 text-left text-sm font-medium text-slate-500">
               <p>VVCE Staff ID: <span className="text-slate-800 font-bold">W-{(profile.id || '').slice(0, 4).toUpperCase()}</span></p>
               <p>Department: <span className="text-slate-800 font-bold">{profile.branch || 'Hostel Administration'}</span></p>
            </div>
          </div>
        </div>
        ...
        <form onSubmit={handleSave} className="lg:col-span-2 bg-white rounded-3xl p-8 shadow-sm border border-slate-200">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-2 col-span-2">
              <label className="text-[10px] font-black text-slate-500 uppercase ml-1">Administrative Dept / Branch</label>
              <input disabled={!isEditing} type="text" value={profile.branch} onChange={e => setProfile({...profile, branch: e.target.value})} className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:border-[#2E8B57] disabled:opacity-70" placeholder="e.g. Hostel Administration" />
            </div>
          </div>
